...
5. Create another `For` loop with iterator `i` to loop over all rows in the spreadsheet. 

> For every increment of `i`, the script interrogates the Ticker column on each row with decision statements similar to the original script to determine ticker volumes, starting and ending prices. The use of independent iterators allows the use of `i` to iterate over rows, while `tickerIndex` is used to store values into the output arrays and participate in decision statements. Because we want to loop through the worksheet only once, we also need `tickerIndex` to increase before the next unique ticker is encountered, therefore an additional line was added to increase `tickerIndex` by 1 if the following conditions are satisfied: 1) the cell in the first column matches `tickers(tickerIndex)` and 2) if the next row does not match `tickers(tickerIndex)`. This means `tickerIndex` increases 12 times in one loop, once per new ticker and storing data for each unique ticker. 
```
For i = 2 To RowCount
